...
**Language**: the conversation follows the user; `spec.md`, `context.md` and the feature slug are English (SKILL.md Critical Rule 6). Translate what the user said into English requirements as you record them — quote the user's language only for a UI label, an error message or a domain term with no English equivalent.
...
### 1.5. Size by scope (structural, before writing)

**A line that is neither an acceptance criterion nor a decision does not belong in `spec.md`.**
The spec is re-read on every planning turn and by every worker and the Verifier by section; every
paragraph of survey narrative is repaid on each read.

- **Small / Medium — ≤ ~8 kB**: Problem (≤5 lines) · numbered WHEN/THEN ACs, each with a precise
  outcome · Out of Scope · a short Assumptions table · the traceability table (with each AC's
  proof, see the template). No user-story prose beyond one line per story, no goals list that
  restates the ACs, no success criteria that restate the ACs.
- **Measurements, versions, grep evidence, `file:line` surveys** never live in `spec.md`. They go
  to `design.md` (§ *Spike results*) or, when there is a lot of it, to a `spike.md` annex in the
  feature folder — read by Design and by the Verifier when an AC's proof points there, never by
  workers and never on every planning turn. The spec cites the annex in one line.
- **Large / Complex** keep the full template ([context-limits.md](context-limits.md) caps apply).
- Over the cap for the scope → move text out before confirming; do not raise the scope to fit
  the text.
...
| -------------- | ----------- | ----- | ------ | ------- |
| [FEAT]-01      | P1: [Story] | test  | Design | Pending |
| [FEAT]-02      | P1: [Story] | gate  | Design | Pending |
| [FEAT]-03      | P2: [Story] | probe: `<command>` | -      | Pending |
...
(the default); `gate` — the exit code of a named gate/build/CI command is the evidence (e.g. "CI
runs without `dist`"), nothing more is invented; `probe: <command>` — a one-off command whose
output is the evidence, named here or in `tasks.md`/`design.md`. **≤ 3 probes per feature**; a
fourth means the AC should be a `test` or a `gate`. An AC without a proof, or a `probe` with no
command, is a spec-precision gap the Verifier reports — never a probe it improvises.
...
- **Requirement IDs are mandatory** — Every story maps to trackable IDs, each with its proof (`test` | `gate` | `probe: <cmd>`)
- **Size by scope** — Small/Medium spec ≤ ~8 kB; evidence and surveys go to `design.md`/`spike.md`, never the spec

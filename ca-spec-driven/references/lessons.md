...
**Language**: every lesson (`--text`, `--grounding`, scope names) is English (SKILL.md Critical Rule 6) — confirmed lessons are loaded at every Specify and Design.
...
For each signal, phrase the lesson as **one terse, actionable, codebase-general sentence** — a rule a future feature could apply, not a restatement of this bug.

**Project-local, mandatory before you phrase anything**: read the project's `.specs/lessons-vocabulary.md` when present, and first search for an existing lesson that already says it:

```bash
python3 scripts/lessons.py list --status all --query "<keyword>"
```

If one matches in meaning, **reuse its `text` verbatim** — only `--feature` and `--source` change. That is what makes recurrence count. Phrasing it your own way silently creates a second candidate, and neither ever promotes. The origin project reached 21 candidates and zero confirmed lessons exactly that way before consolidating.
...
## PROMOTE — a confirmed lesson must become a gate (project-local)

Promotion is a **trigger, not an endpoint**. A lesson that stays as text is guidance that only applies while someone is running this skill — it does nothing for an ordinary edit outside the flow. In the origin project the rules that actually held (a contract-enum rule, a closed UI primitive, branch-only-in-worktree) all ended as something that blocks.

When `add` prints `PROMOTED to confirmed`, take the highest rung that honestly fits and do it in the same commit:

| Rung | Mechanism | Fits when |
| ---- | --------- | --------- |
| 1 | An edit-time guard the harness runs before the write (in Claude Code, a PreToolUse hook in `.claude/hooks/`) | The violation is detectable in the edit itself (naming, forbidden import, wrong path) |
| 2 | A test that fails on violation (a contract test, or a component test that asserts the primitive's shape, are the models) | The violation is detectable by scanning the tree or the contract |
| 3 | A line in the area handbook (the project's testing handbook, its architecture handbook) or in the project's `CLAUDE.md` | The rule needs human judgement — most evidence-quality rules land here |
...
Do **not** delete the lesson from the store after gating it; recurrence data is what tells the maintainer the gate works.
...
Deduplication is exact-after-normalization (lowercase, punctuation-stripped) — there are no embeddings (stdlib-only, zero-dependency by design). Near-duplicate lessons phrased differently will not merge and will each sit as separate candidates that never promote. Mitigation: follow the phrasing rules above and the closed vocabulary in the project's `.specs/lessons-vocabulary.md` when present. A future version may add embedding-based dedup.

The key is `signal + normalized text`, so the same rule recorded under two different signals never merges either. Pick the signal from the table above by what the Verifier reported.
